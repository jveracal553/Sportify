...
const GridCanchitaItem = ({ product }) => {
...
function GridCanchita() {
  const [GridCanchitaItem, setGridCanchitaItem] = useState([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    axios.get('https://fakeGridCanchitaapi.com/products').then(({ data }) => {
      setLoading(false);
      setGridCanchitaItem(data);
    });
  }, []);
      ...
      {loading ? (
        ...
        <Box p={2}>
          <SimpleGrid columns={{ base: 1, md: 5 }} spacing={4} mt={4}>
            {GridCanchitaItem.map((item) => {
              return <GridCanchitaItem key={item.id} product={item} />;
            })}
          </SimpleGrid>
        </Box>
      )}
